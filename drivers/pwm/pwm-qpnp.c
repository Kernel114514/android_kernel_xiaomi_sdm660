					...
					struct qpnp_pwm_chip *chip)
{
	...
	index = of_property_match_string(of_node, "reg-names", QPNP_LPG_CHANNEL_BASE);
	if (index < 0) {
		dev_err(&pdev->dev, "Couldnt find channel's base addr name index\n");
		return -ENODEV;
	}

	prop = of_get_address(of_node, index, &addr_size, &addr_flags);
	if (!prop) {
		dev_err(&pdev->dev, "Couldnt parse channel's base addr from DT\n");
		return -ENODEV;
	}
	...
	index = of_property_match_string(of_node, "reg-names", QPNP_LPG_LUT_BASE);
	if (index < 0) {
		chip->flags |= QPNP_PWM_LUT_NOT_SUPPORTED;
	} else {
		prop = of_get_address(of_node, index, &addr_size, &addr_flags);
